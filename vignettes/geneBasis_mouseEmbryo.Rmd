...
Let's compare UMAP plots when using whole transcriptome (already pre-saved) and using the selected panel (UMAP-coordinates can be calculated using `get_umap_coordinates`).

```{r comp-umaps, fig.wide = TRUE, fig.cap="Average expression per cell type."}
...
umaps_selection = get_umap_coordinates(sce, genes = genes, batch = "sample")
