...
#' Returns assigned celltype labels for the Selection graph.
#'
...
#' @param ... Additional arguments you can pass to get_DE_genes.
#' @return
#' @export
#'
#' @examples
...
get_celltype_mapping = function(sce , genes.selection , batch = NULL, n.neigh = 5 , nPC.selection = NULL, cosine = F, return.stat = T, which_genes_to_use = "all", ...){
  sce = .prepare_sce(sce)
  if (.check_celltype_in_sce(sce) & .general_check_arguments(args) & .check_batch(sce , batch) & .check_genes_in_sce(sce , genes.selection)){
    if (which_genes_to_use == "DE"){
      markers = get_DE_genes(sce, ...)
      if (is.null(markers)){
        ...
        if (length(genes.selection) < 2){
          stop("Less than 2 genes are selected as DE between celltypes - celltype mapping is not possible. Consider option 'all' or chnage settings for identifying DE genes.")
          return(F)
        }
        else {
...
#' get_fraction_mapped_correctly
#' Returns stat on the celltype mapping - diagonal values for the confusion matrix.
#'
#' @param mapping Data.frame representing celltype mapping. Should contain fields 'celltype' and 'mapped_celltype'.
#'
#' @return
#' @export
#'
...
#' sce$celltype = as.character(sample.int(5, n_col, replace = TRUE))
#' genes.selection = sample( rownames(sce) , 20)
#' mapping = get_celltype_mapping(sce, genes.selection = genes.selection)
#' out = get_fraction_mapped_correctly(mapping$mapping)
